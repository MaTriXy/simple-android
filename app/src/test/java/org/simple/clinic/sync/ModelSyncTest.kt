...
import org.simple.clinic.sync.ModelSyncTest.SyncOp.PULL
import org.simple.clinic.sync.ModelSyncTest.SyncOp.PUSH
...
class ModelSyncTest {
  ...
  fun <T : Any, P : Any> `errors during push should not affect pull`(
      ...
      supportedSyncOps: Set<SyncOp>
  ) {
    if ((PULL in supportedSyncOps).not()) {
      return
    }
  ...
  fun <T : Any, P : Any> `errors during pull should not affect push`(
    ...
    if ((PUSH in supportedSyncOps).not()) {
      return
    }
